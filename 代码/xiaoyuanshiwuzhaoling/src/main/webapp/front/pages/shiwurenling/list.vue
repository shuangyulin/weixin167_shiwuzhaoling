...
<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
		...
		<view class="search-form round">
			...
			<input v-model="searchForm.shiwurenlingName" type="text" placeholder="失物认领名称"></input>
		</view>
		<view class="action">
			<button @tap="search"
	...
	<view class="list">
		...
		<view v-for="(item,index) in list " v-bind:key="index" class="listm flex flex-between">
			<view class="listmr">
				...
				<view style="margin-left: 480rpx;" v-if="item.yesnoTypes == 1">
					<text v-if="user.id != item.yonghuId" class="cuIcon-edit"
						@click.stop="onUpdateTap(item,2)">同意</text>
					<text v-if="user.id != item.yonghuId" class="cuIcon-delete"
						@click.stop="onUpdateTap(item,3)">拒绝</text>
				</view> 
			</view>
		</view>
	</view>
	...
	export default {
		...
		async onShow() {
			...
			_this.btnColor = _this.btnColor.sort(() => {
	   		...
	   		//当前表的 审核 字段 字典表查询方法
		  	let yesnoTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'yesno_types'
		  	});
		  	this.yesnoTypesList = yesnoTypes.data.list;
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.shiwurenlingName = "";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				let res = await this.$api.page(`shiwurenling`, params);
				// 如果是第一页数据置空
			...
			onUpdateTap(item,type) {
				...
				uni.showModal({
					title: '提示',
					content: '是否确认'+tishi+"审核结果",
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.update('shiwurenling', params);
							if(type == 2){
								let data = {
									id: item.xiaoyuanshiwuzhaolingId,
									statusTypes: 2
								}
								await _this.$api.update('xiaoyuanshiwuzhaoling', data);
							}
							_this.$utils.msg("状态修改成功")
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('shiwurenling', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
							_this.mescroll.resetUpScroll()
						}
			...
			async search() {
				...
				if (this.searchForm.shiwurenlingName) {
					searchForm['shiwurenlingName'] = this.searchForm.shiwurenlingName
				}
				let res = await this.$api.page(`shiwurenling`, searchForm);
				// 如果是第一页数据置空
