...
<!-- text后缀 -->
            <view
                ...
                <textarea :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0, 102, 51, 1)","borderRadius":"0px","color":"rgba(0, 102, 51, 1)","textAlign":"left","borderWidth":"0 0 6rpx","fontSize":"28rpx","borderStyle":"solid","height":"200rpx"}'
                          :disabled="ro.shiwurenlingText" v-model="ruleForm.shiwurenlingText" placeholder="认领凭据"/>
            </view>
					<view class="btn">
				<button
...
<script>
	...
	export default {
		...
		async onLoad(options) {
			this.ruleForm.xiaoyuanshiwuzhaolingId = options.shiwurenlingId
		/*下拉框*/
			let yesnoTypesParams = {
				page: 1,
				limit: 100,
				dicCode: 'yesno_types',
			}
			let yesnoTypes = await this.$api.page(`dictionary`, yesnoTypesParams);
			this.yesnoTypesOptions = yesnoTypes.data.list
			...
			if (options.id) {
				...
				// 获取信息
				let res = await this.$api.info(`shiwurenling`, this.ruleForm.id);
				this.ruleForm = res.data;
                uni.setStorageSync('ShiwurenlingState', true);
			}
			// 跨表
			// this.styleChange()
		},
		methods: {
			// 下拉变化
            yesnoTypesChange(e) {
            this.yesnoTypesIndex = e.target.value
            this.ruleForm.yesnoValue = this.yesnoTypesOptions[this.yesnoTypesIndex].indexName
            this.ruleForm.yesnoTypes = this.yesnoTypesOptions[this.yesnoTypesIndex].codeIndex
        },
			...
			async onSubmitTap() {
				...
				if ((!this.ruleForm.shiwurenlingText)) {
					this.$utils.msg(`详情不能为空`);
					return
				}
				if (this.ruleForm.id) {
					await this.$api.update(`shiwurenling`, this.ruleForm);
				} else {
					await this.$api.save(`shiwurenling`, this.ruleForm);
				}
