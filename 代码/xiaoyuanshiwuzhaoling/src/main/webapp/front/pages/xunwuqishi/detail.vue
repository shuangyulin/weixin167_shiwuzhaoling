		...
		<view class="container">
			...
			<view class="detail-content">
				...
				<view class="detail-list-item"
					...
					<view class="text" :style='{"color":"rgba(0, 102, 51, 1)","textAlign":"center","fontSize":"24rpx"}'>
						{{detail.shiwuzhaolingValue}}</view>
				</view>
				<view class="detail-list-item"
					:style='{"boxShadow":"0 0 4rpx 0 #006633 insta","margin":"0 60rpx 0 60rpx","borderColor":"rgba(218, 218, 218, 1)","backgroundColor":"#fff","borderRadius":"0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"68rpx"}'>
					<view class="lable"
			...
			<view class="time-content"
				...
				<view class="content" style="line-height: 50upx;letter-spacing: 5upx;"
					...
					<rich-text :nodes="detail.xunwuqishiContent"></rich-text>
				</view>
			</view>
			...
			<view class="bottom-content cu-bar bg-white tabbar border shop">
				...
				<view style="text-align: right;">
					<button @tap="onUpdateXinxiTypes(detail)" style="margin-right: 10upx;min-width: 180upx;"
						class="btn-submit cu-btn round bg-red" v-if="user.id == detail.yonghuId && detail.statusTypes == 1"
						:style="{width:'auto',borderRadius:'40rpx',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[2],borderColor:btnColor[2]}">改为已找回</button>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('xunwuqishiLiuyanState')
			if (cleanType) {
				uni.removeStorageSync('xunwuqishiLiuyanState')
				this.mescroll.num = 1
		...
		methods: {
			...
			async init() {
				...
				// 轮播图片
				this.swiperList = this.detail.xunwuqishiPhoto ? this.detail.xunwuqishiPhoto.split(",") : [];
			},
			...
			async upCallback(mescroll) {
				let res = await this.$api.list('xunwuqishiLiuyan', {
					page: mescroll.num,
					limit: mescroll.size,
					xunwuqishiId: this.id
				});
				// 如果是第一页数据置空
			...
			onSubmitLingyang(id){
				...
				this.$api.save('lingyang', params);
				this.$utils.msg("添加成功")
			},
			onUpdateXinxiTypes(detail){
				detail.statusTypes = 2
				this.$api.update('xunwuqishi', detail);
				uni.redirectTo({
				    url: '/pages/xunwuqishi/detail?id='+detail.id
				});
			},
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../xunwuqishiLiuyan/add-or-update?xunwuqishiId=${this.id}`)
			},
		}
	}
</script>
